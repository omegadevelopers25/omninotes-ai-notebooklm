/**
 * OmniNotes AI - Application Frontend Logic
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const elements = {
        ...
        noteToolbar: document.getElementById('noteToolbar'),
        ...
        btnOpenHistoryDrawer: document.getElementById('btnOpenHistoryDrawer'),
    ...
    function setupEventListeners() {
        ...
        elements.btnRemoveApiKey.addEventListener('click', () => {
            ...
            updateApiKeyUI();
            elements.apiKeyModal.classList.remove('active');
            showToast('API Key removed. Using Smart Offline engine.', 'info');
        });
    ...
    async function fetchSource(endpoint, payload, onSuccess) {
        ...
        try {
            ...
            showToast(`Added source: ${data.source.title}`, 'success');
        ...
        if (count === 0) {
            elements.sourcesList.innerHTML = `
                <div class="empty-queue-state">
                    ...
                    <span>Add web links, videos, or files above to synthesize multi-source notes!</span>
                </div>
            `;
            return;
        }
            ...
            <div class="source-card">
                <div class="source-info">
                    ...
                    <div class="source-meta">
                        ...
                        <span class="source-sub">${src.domain || 'Source'} • ~${src.wordCount} words</span>
                    </div>
                </div>
                <button class="source-remove" data-index="${index}">
    ...
    async function handleGenerateNotes() {
        ...
        elements.btnGenerateNotes.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Synthesizing Notes...';
        ...
        elements.noteDisplayArea.innerHTML = `
            <div class="empty-note-state">
                ...
                <h3>Generating NotebookLM Master Notes...</h3>
                <p>Synthesizing ${state.sources.length} sources into clean, structured Markdown ready for NotebookLM.</p>
            </div>
        `;
        ...
        try {
            ...
            elements.noteDisplayArea.innerHTML = `
                <div class="empty-note-state">
                    <h3 style="color: var(--danger);">Error Generating Notes</h3>
                    <p>${err.message}</p>
                </div>
            `;
        } finally {
            elements.btnGenerateNotes.disabled = false;
            elements.btnGenerateNotes.innerHTML = '<i class="fa-solid fa-wand-magic-sparkles"></i> Generate NotebookLM Notes';
        }
    }

    function renderGeneratedNote(note) {
        elements.noteToolbar.style.display = 'flex';
    ...
    function copyNoteForNotebookLM() {
        ...
        navigator.clipboard.writeText(state.currentNote.markdown).then(() => {
            showToast('📋 Copied formatted note! Ready for NotebookLM paste.', 'success');
            
            // Ask user if they want to launch NotebookLM in new tab
            if (confirm("Note copied to clipboard!\n\nWould you like to open Google NotebookLM now to paste this into your notebook?")) {
    ...
    function exportFile(format) {
        ...
        if (format === 'md') {
            ...
            content = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${state.currentNote.title}</title><style>body{font-family:sans-serif;line-height:1.6;max-width:800px;margin:40px auto;padding:0 20px;}blockquote{border-left:4px solid #10b981;padding-left:16px;color:#047857;}</style></head><body>${marked.parse(state.currentNote.markdown)}</body></html>`;
            filename += '.html';
            mimeType = 'text/html';
        }
    ...
    // Speech Synthesizer for Audio Podcast Script
    ...
    async function openHistoryDrawer() {
        ...
        try {
            ...
            if (data.notebooks && data.notebooks.length > 0) {
                ...
                elements.historyList.innerHTML = '<p style="color:var(--text-dim); text-align:center; padding:20px;">No saved notebooks yet.</p>';
            }
        } catch (e) {
    ...
    function updateApiKeyUI() {
        if (state.apiKey) {
            ...
            elements.apiKeyBadge.style.color = '#34d399';
        } else {
            elements.apiKeyBadge.textContent = 'Set API Key';
            elements.apiKeyBadge.style.color = 'var(--text-muted)';
        }
    }
